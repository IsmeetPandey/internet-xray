...
function render(data) {
  ...
  document.querySelector('#metrics').innerHTML = [
    ['Requests', data.request_count], ['Third-party', data.third_party_request_count],
    ['Failed', data.failed_request_count], ['Known data', bytes(data.known_response_bytes)]
  ...
  const timingRows = [['DNS', timing.dns_ms], ['TCP', timing.tcp_ms], ['TLS', timing.tls_ms], ['TTFB', timing.ttfb_ms], ['DOM ready', timing.dom_content_loaded_ms], ['Load', timing.load_event_ms]];
  document.querySelector('#timing').innerHTML = timingRows.map(([name, value]) => `<div class="timing-item"><div class="name">${name}</div><div class="num">${ms(value)}</div></div>`).join('');

  const resources = Object.entries(data.resource_counts || {}).sort((a,b) => b[1]-a[1]);
  const maxResource = resources[0]?.[1] || 1;
  document.querySelector('#resources').innerHTML = resources.length ? resources.map(([name,count]) => `<div class="bar-row"><span>${esc(name)}</span><div class="track"><div class="fill" style="width:${(count/maxResource)*100}%"></div></div><strong>${count}</strong></div>`).join('') : '<div class="muted">No response records.</div>';

  const hosts = Object.entries(data.host_counts || {}).sort((a,b) => b[1]-a[1]).slice(0, 14);
  document.querySelector('#hosts').innerHTML = hosts.length ? hosts.map(([host,count]) => `<div class="host"><span>${esc(host)}</span><span class="count">${count}</span></div>`).join('') : '<div class="muted">No hosts recorded.</div>';

  const security = data.security_headers || {};
  const missing = data.missing_security_headers || [];
  document.querySelector('#security').innerHTML = [...Object.entries(security).map(([name]) => `<div class="host"><span>${esc(name)}</span><span class="count status-ok">present</span></div>`), ...missing.map(name => `<div class="host"><span>${esc(name)}</span><span class="count status-bad">not observed</span></div>`)].join('') || '<div class="muted">No tracked headers were observed.</div>';

  const failures = (data.requests || []).filter(r => r.failed || (r.status && r.status >= 400)).slice(0, 10);
  document.querySelector('#failures').innerHTML = failures.length ? failures.map(r => `<div class="failure"><strong>${esc(r.host || 'unknown')}</strong><span>${esc(r.failure || `HTTP ${r.status}`)}</span></div>`).join('') : '<div class="muted">No failed requests or 4xx/5xx responses captured.</div>';
